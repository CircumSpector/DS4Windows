        ...
        public void UpdateLightBar(DS4Device device, int deviceNum)
        {
            ...
            if (!defaultLight && !useForceLight && useLightRoutine)
            {
                ...
                else
                {
                    ...
                    if (rainbow > 0)
                    {
                        ...
                        if (now >= oldnow +
                            TimeSpan.FromMilliseconds(10)) //update by the millisecond that way it's a smooth transition
                        {
                            oldnow = now;
                            if (device.IsCharging())
                                counters -= 1.5 * 3 / rainbow;
                            else
                                counters += 1.5 * 3 / rainbow;
                        }
                ...
                if (device.GetBattery() <= lightModeInfo.FlashAt && !defaultLight && !device.IsCharging())
                {
                    ...
                    if (lightModeInfo.FlashType == 1)
                    {
                        ...
                        if (!fadewatches.IsRunning)
                        {
                            var temp = fadedirection;
                            fadedirection = !temp;
                            fadewatches.Restart();
                        ...
                        {
                            var elapsed = fadewatches.ElapsedMilliseconds;
            ...
            if (useLightRoutine)
            {
                ...
                */
                ...
                {
                    if (useForceLight && forcedFlash > 0)
                    {
                        lightState.LightBarFlashDurationOff =
                            lightState.LightBarFlashDurationOn = (byte)(25 - forcedFlash);
