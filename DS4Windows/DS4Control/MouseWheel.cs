...
namespace DS4Windows
{
    internal class MouseWheel
    {
        ...
        public void touchesMoved(TouchpadEventArgs arg, bool dragging)
        {
            ...
            var coefficient = ProfilesService.Instance.ActiveProfiles.ElementAt(deviceNumber).ScrollSensitivity / 100.0;
            ...
            double touchXDistance = T1.hwX - T0.hwX,
                touchYDistance = T1.hwY - T0.hwY,
                touchDistance = Math.Sqrt(touchXDistance * touchXDistance + touchYDistance * touchYDistance);
            ...
            {
                yAction = yAction < 0
                    ? yAction * -1 * Global.outputKBMMapping.WHEEL_TICK_DOWN
                    : yAction * Global.outputKBMMapping.WHEEL_TICK_UP;

                xAction = xAction < 0
                    ? xAction * -1 * Global.outputKBMMapping.WHEEL_TICK_DOWN
