...
namespace DS4Windows.InputDevices
{
    public class SwitchProDevice : DS4Device
    {
        ...
        public SwitchProDevice(HidDeviceV3 hidDevice,
            ...
            base(hidDevice, disName, featureSet)
        {
            ...
            leftStickXData.max = SAMPLE_STICK_MAX;
            leftStickXData.min = SAMPLE_STICK_MIN;
            leftStickXData.mid = SAMPLE_STICK_MID;

            leftStickYData.max = SAMPLE_STICK_MAX;
            leftStickYData.min = SAMPLE_STICK_MIN;
            leftStickYData.mid = SAMPLE_STICK_MID;

            rightStickXData.max = SAMPLE_STICK_MAX;
            rightStickXData.min = SAMPLE_STICK_MIN;
            rightStickXData.mid = SAMPLE_STICK_MID;

            rightStickYData.max = SAMPLE_STICK_MAX;
            rightStickYData.min = SAMPLE_STICK_MIN;
            ...
            */
            ...
            unchecked
            {
                ...
                while (!exitInputThread)
                {
                    ...
                    tempAxis = tempAxis > leftStickXData.max ? leftStickXData.max :
                        tempAxis < leftStickXData.min ? leftStickXData.min : tempAxis;
                    currentState.LX = (byte)((tempAxis - leftStickXData.min) /
                        (double)(leftStickXData.max - leftStickXData.min) * 255);

                    tempAxis = ((stick_raw[1] >> 4) | (stick_raw[2] << 4)) - leftStickOffsetY;
                    tempAxis = tempAxis > leftStickYData.max ? leftStickYData.max :
                        tempAxis < leftStickYData.min ? leftStickYData.min : tempAxis;
                    currentState.LY =
                        (byte)((((tempAxis - leftStickYData.min) / (double)(leftStickYData.max - leftStickYData.min) -
                                 0.5) * -1.0 + 0.5) * 255);
                    ...
                    tempAxis = tempAxis > rightStickXData.max ? rightStickXData.max :
                        tempAxis < rightStickXData.min ? rightStickXData.min : tempAxis;
                    currentState.RX = (byte)((tempAxis - rightStickXData.min) /
                        (double)(rightStickXData.max - rightStickXData.min) * 255);

                    tempAxis = ((stick_raw2[1] >> 4) | (stick_raw2[2] << 4)) - rightStickOffsetY;
                    tempAxis = tempAxis > rightStickYData.max ? rightStickYData.max :
                        tempAxis < rightStickYData.min ? rightStickYData.min : tempAxis;
                    //cState.RY = (byte)((tempAxis - STICK_MIN) / (STICK_MAX - STICK_MIN) * 255);
                    currentState.RY =
                        (byte)((((tempAxis - rightStickYData.min) /
        ...
        private void RunUSBSetup()
        {
            ...
            byte[] modeSwitchCommand = { 0x3F };
            Subcommand(0x03, modeSwitchCommand, 1);
            ...
            data[0] = 0x80;
            data[1] = 0x03; // 3Mbit baud rate
            //result = hidDevice.WriteAsyncOutputReportViaInterrupt(data);
            result = hDevice.WriteOutputReportViaControl(data);
            //Thread.Sleep(2000);

            data[0] = 0x80;
            data[1] = 0x02; // Handshake at new baud rate
            result = hDevice.WriteOutputReportViaControl(data);
            //Thread.Sleep(1000);
            ...
            data[0] = 0x80;
            data[1] = 0x4; // Prevent HID timeout
            result = hDevice.WriteOutputReportViaControl(data);
            //result = hidDevice.WriteOutputReportViaInterrupt(command, 500);
        }
        ...
        private void EnableFastPollRate()
        {
            ...
            byte[] tempArray = { 0x30 };
            Subcommand(0x03, tempArray, 1, true);
            //Thread.Sleep(1000);
        }
        ...
        public void PrepareRumbleData(byte[] buffer)
        {
            ...
            var idx = (int)(currentLeftAmpRatio * AMP_LIMIT_MAX);
            var entry = compiledRumbleTable[idx];
            var amp_high = entry.high;
            var amp_low = entry.low;
        ...
        public void CalibrationData()
        {
            ...
            tmpBuffer = Subcommand(0x10, command, 5, true);
        ...
        public void WriteReport()
        {
            ...
            bool dirty;
            var tempRatio = CurrentHaptics.RumbleState.RumbleMotorStrengthLeftHeavySlow / 255.0;
